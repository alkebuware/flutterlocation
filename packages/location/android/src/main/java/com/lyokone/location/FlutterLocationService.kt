...
class FlutterLocationService : Service(), PluginRegistry.RequestPermissionsResultListener {
    ...
    // Store method channel parameter here when requesting permissions
    ...
    fun checkBackgroundPermissions(foregroundOnly: Boolean = false): Boolean {
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && !foregroundOnly) {
            activity?.let {
    ...
    fun requestBackgroundPermissions(foregroundOnly: Boolean = false) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            activity?.let {
                val array = mutableListOf(Manifest.permission.ACCESS_FINE_LOCATION)
                if (!foregroundOnly) {
                    array.add(Manifest.permission.ACCESS_BACKGROUND_LOCATION)
                }
                this.foregroundOnly = foregroundOnly
                ActivityCompat.requestPermissions(
                    it, array.toTypedArray(),
    ...
    override fun onRequestPermissionsResult(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && requestCode == REQUEST_PERMISSIONS_REQUEST_CODE && permissions!!.size == permissionsSize &&
            permissions[0] == Manifest.permission.ACCESS_FINE_LOCATION && (foregroundOnly || permissions[1] == Manifest.permission.ACCESS_BACKGROUND_LOCATION)
        ) {
            if (grantResults!![0] == PackageManager.PERMISSION_GRANTED && (foregroundOnly || grantResults[1] == PackageManager.PERMISSION_GRANTED)) {
    ...
    private fun shouldShowRequestBackgroundPermissionRationale(): Boolean =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && !foregroundOnly) {
            activity?.let {
